...
export function hasHOC(rawData) {
    ...
    return (data.split('\n').filter(function (line) {
        ...
        var isComp = new RegExp("(function|class) +" + ref + "\\W").test(line);
        var isCompInVar = new RegExp(" *" + ref + " += +(function|class) +").test(line);
        var isArrowFunc = new RegExp(" *" + ref + "(: *\\w+ *)? += +\\(.*=>").test(line);
...
function getRef(data) {
    ...
    var ref = (data.replace(/ /g, '').match("exportdefault*([^\\n|;]*)") ||
        [])[1];
    var prevRef = (data.match(new RegExp(escapeRegex(ref) + " += +(\\w+)($| |;|\\n)")) || [])[1];
    return prevRef || ref;
}
